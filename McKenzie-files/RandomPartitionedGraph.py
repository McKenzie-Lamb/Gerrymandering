...
def MakeNXGraphOld(size = 100, dem_mean = 0.5, dem_sd = 0.3):
...
#    H = nx.triangular_lattice_graph(size, size)
    filename = 'whole_map_no_discontiguos.gpickle'
...
#    G = nx.convert_node_labels_to_integers(H)
    ...
    for node in G.nodes():            
        ...
        distance_dem_mean = 4500 * (dfc / (max_dfc))
...
#G, parts = MakeGraphPartition(size = 10000, num_parts = 4)
