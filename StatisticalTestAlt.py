...
def simulation(total_percent_dem, dist_list, number_district, number_samples):  #generates sample lists of random districts w/ same demographics as given state
    ...
    while len(test_list) < number_samples:
        simulated_set = random.sample(dist_list, number_district) #generates list of random districts
        ...
        - total_percent_dem) > 0.01:    #allow for margin of error of 1% for the demographic breakdown of the simualated set
            TweakSample(simulated_set, total_percent_dem, dem_prop)            
        total_dem_seat = 0
...
def TweakSample(sample, target_dem, current_dem_prop):
    new_cutoff = 0
    while abs(current_dem_prop - total_percent_dem) > 0.01:
        #Slice out the districts with democratic proportions greater than 
        while sorted_dist_list[new_cutoff].dem_votes < current_dem_prop:
            new_cutoff += 1
        random.choice(sorted_dist_list[cutoff:])
...
test_list = simulation(total_percent_dem, other_dist_list, number_district, number_samples, sorted_dist_list)
