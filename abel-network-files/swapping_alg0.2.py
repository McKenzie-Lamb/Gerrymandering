...
def _select_sded(graph, districts_graphs):
    start_district = random.randint(0, len(districts_graphs) - 1)
    end_district = _get_node_district(random.sample(nx.node_boundary(graph, districts_graphs[start_district]), 1)[0], districts_graphs)
    return start_district, end_district


def _get_node_district(node, districts_graphs):
    for i in districts_graphs.keys():
        if node in districts_graphs[i]:
            return i
