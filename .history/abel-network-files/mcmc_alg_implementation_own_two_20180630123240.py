...
def gather_districts_data(districts_graphs):
    for i in range(len(districts_graphs)):
        ...
        color = districts_graphs[i].new_graph_property('vector<double>')
        districts_graphs[i].graph_properties["color"] = color
...
def adjust_color(districts_graphs):
    for i in range(len(districts_graphs)):
        if districts_graphs[i].graph_properties["dem_vote"] > districts_graphs[i].graph_properties["rep_vote"]:
            districts_graphs[i].graph_properties["color"] = (0,0,255,1)
        else:
            districts_graphs[i].graph_properties["color"] = (255,0,0,1)
...
for i in range(len(districts_graphs)):
    gt.graph_draw(
                  ...
                  bg_color=(255, 255, 255, 1), vertex_color = districts_graphs[i].graph_properties["color"] = (255,0,0,1))
