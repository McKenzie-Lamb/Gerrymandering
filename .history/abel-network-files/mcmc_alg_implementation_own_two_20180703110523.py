...
def create_graph_views(district_total_no):
    ...
    for i in range(district_total_no):
        main_graph = gt.GraphView(graph)
        graph_view_check = main_graph.new_vertex_property("bool")
        matched_vertices = gt.find_vertex(graph, district_no, i)
        for j in matched_vertices:
            graph_view_check[j] = True
        graph_view = gt.GraphView(main_graph, vfilt=graph_view_check)
        graph_view.base.vp.valid = graph_view_check
...
def get_non_adjacent_v(labels_in_boundaries, graph):
    list_to_swap = random.sample(
        labels_in_boundaries, random.randint(2, 4)) # Prob for choosing from boundaries
    index_to_del = list()
        ...
        for i in changes.keys():
            ...
            for j in vertex_to_add[i]:
                if len(vertex_to_add[i]) == 0:
                    break
                districts_graphs[i].base.vp.valid[j] = True
            for j in vertex_to_delete[i]:
                if len(vertex_to_delete[i]) == 0:
                    break
                districts_graphs[i].base.vp.valid[j] = False
            districts_graphs[i].base = gt.GraphView(districts_graphs[i], skip_vfilt=True, skip_efilt=True)
            districts_graphs[i] = gt.GraphView(districts_graphs[i].base, vfilt = districts_graphs[i].base.vp.valid)
        return districts_graphs
    else:
        #print("Pop differ")
