...
def draw_state(state, graph, name, pos):
    graph.vp.pos = pos
    state.draw(pos=graph.vp.pos, output="abel-network-files/tmp_alg_states"+str(name)+".png")

def main(no_districts):
    data_folder = Path("abel-network-files/data/")
    images_folder = Path("abel-network-files/images/")

    size_array = np.random.random([20, 2])
    ...
    state.exhaustive_sweep(callback=draw_state(state, graph, random.randint(1,10000), pos))
    state.exhaustive_sweep(callback=draw_state(state, graph, random.randint(1,10000), pos))
    graph.vp.pos = pos
