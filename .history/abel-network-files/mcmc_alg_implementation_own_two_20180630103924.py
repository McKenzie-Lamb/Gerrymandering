...
def random_selection(cp_boundaries):
    list_to_select = cp_boundaries
    while list_to_select:
        index = random.randrange(len(list_to_select))
        elem = list_to_select[index]
        # direct deletion, no search needed
        del list_to_select[index]
    return list_to_select
...
slected_vertices = random_selection(labels_in_boundaries)
