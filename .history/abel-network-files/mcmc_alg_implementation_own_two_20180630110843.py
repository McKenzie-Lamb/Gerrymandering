...
def get_non_adjacent_v(labels_in_boundaries):
    list_to_swap = random.sample(labels_in_boundaries, random.randint(1,len(labels_in_boundaries)//3))
        ...
        for v in range(len(list_to_swap[l])):
            for l_two in range(len(list_to_swap)):
                ...
                for v_two in range(len(list_to_swap[l_two])):
                    if len(gt.shortest_path(graph, list_two_swap[l][v], list_two_swap[l_two][v_two])) == 0:
                        del(list_to_swap, l)
    return list_to_swap
