...
gt.graph_draw(graph, vertex_color = ring_color,vertex_fill_color = color, 
              output = str(main_folder / 'tmp.png'), pos=graph.vp.pos, bg_color=[1,1,1,1])
...
for i in range(100):
    ...
    for j in districts_graphs:
        ...
        if np.sum(comp[0].a) > 0:
            gt.graph_draw(districts_graphs[j], vertex_color = ring_color, vertex_fill_color = color,
