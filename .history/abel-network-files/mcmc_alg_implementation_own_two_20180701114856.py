...
def propose_swap(districts_graphs, proposed_components, graph):
    ...
    for c in proposed_components:
        for v in range(len(c)):
            ...
            neighbor_dindex = graph.vp.nd[c[v]]
            print(added_dem)
            print(neighbor_dindex)
    for d in range(len(districts_graphs)):
        if d == neighbor_dindex:
            new_pop = districts_graphs[d].graph_properties["pop"] + added_pop
            new_rep = districts_graphs[d].graph_properties["pop"] + added_rep
            new_dem = districts_graphs[d].graph_properties["pop"] + added_dem
