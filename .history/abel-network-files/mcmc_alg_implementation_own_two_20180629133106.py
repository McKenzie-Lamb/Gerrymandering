...
def get_cp_boundaries(turned_on_graphs):
    for g in turned_on_graphs:
        cp_label, hist = gt.label_components(g)
        labels = set(cp_label.a)
        print(labels)
        to_delete = g.new_vertex_property('bool')
        for l in labels:
...
get_connected_components(turned_on_graphs)