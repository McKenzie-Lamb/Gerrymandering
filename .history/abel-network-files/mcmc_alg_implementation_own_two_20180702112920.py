...
def propose_swap(districts_graphs, proposed_components, graph, labels_in_boundaries):
    ...
    if similar_pop == True:
        ...
        for i in changes.keys():
            ...
            for j in vertex_to_delete.keys():
                print(vertex_to_delete[j])
                districts_graphs[j].remove_vertex(vertex_to_delete[j])
            for j in vertex_to_add.keys():
                rint(vertex_to_delete[j])
                districts_graphs[j].add_vertex(vertex_to_add[j])
