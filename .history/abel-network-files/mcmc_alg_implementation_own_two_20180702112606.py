...
def get_non_adjacent_v(labels_in_boundaries, graph):
    list_to_swap = random.sample(
        labels_in_boundaries, random.randint(2, (2*len(labels_in_boundaries))//3))
    index_to_del = list()
...
def propose_swap(districts_graphs, proposed_components, graph, labels_in_boundaries):
    ...
    if similar_pop == True:
        print("Similar pop")
        for i in changes.keys():
            copy_of_graph = gt.GraphView(districts_graphs[i].base)
            print(len(copy_of_graph.get_vertices()))
            to_keep = copy_of_graph.new_vertex_property('bool')
            ...
            for i in vertex_to_add.keys():
                for j in range(len(vertex_to_add[i])):
                    copy_of_graph.add_vertex(graph.vertex(vertex_to_add[i][j]))
            to_keep.set_value(True)
            for i in vertex_to_delete.keys():
                for j in range(len(vertex_to_delete[i])):
                    to_keep[vertex_to_add[i][j]] = False
            districts_graphs[i] = gt.GraphView(copy_of_graph, vfilt=to_keep)
...
for i in range(1):
    ...
    adjust_color(districts_graphs, color, ring_color)
    gt.graph_draw(graph, vertex_fill_color = color, vertex_color = ring_color,
