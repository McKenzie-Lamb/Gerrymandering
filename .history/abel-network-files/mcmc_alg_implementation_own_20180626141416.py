...
for i in graph.vertices():
    ...
    if district_no in districts_data.keys():
        for j in districts_data[i].keys():
            districts_data[i][j] += graph.vp.data[i]
    else:
        districts_data[i] = graph.vp.data[i]
...
for i in districts_data.keys():
    ...
    matched_vertices = gt.find_vertex(graph, prop["district_no"] = i)
    for j in matched_vertices:
        ring_color[i] = ring_color


gt.graph_draw(graph, bg_color=(255, 255, 255, 1), vertex_fill_color=color, pos=graph.vp.pos,
