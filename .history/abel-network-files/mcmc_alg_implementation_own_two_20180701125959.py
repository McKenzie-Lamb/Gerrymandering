...
def turn_off_edges(districts_graphs):
    ...
    for district in range(len(districts_graphs)):
        ...
        selected = edges[np.random.randint(
            edges.shape[0], size=len(edges)//3.5), :]
        for i in selected:
            to_delete[i] = True
...
def get_non_adjacent_v(labels_in_boundaries, graph):
    list_to_swap = random.sample(
        labels_in_boundaries, random.randint(2, len(labels_in_boundaries)//2))
    index_to_del = list()
...
def propose_swap(districts_graphs, proposed_components, graph):
    changes = dict()
    vertex_to_add = dict()
    vertex_to_delete = dict()
    for i in range(len(districts_graphs)):
        changes[i] = [districts_graphs[i].graph_properties["pop"].copy(),
                      districts_graphs[i].graph_properties["rep_vote"].copy(),
                      districts_graphs[i].graph_properties["dem_vote"].copy()]
        vertex_to_add[i] = []
        vertex_to_delete[i] = []
    for c in proposed_components:
        ...
        for v in range(len(c)):
            ...
            vertex_to_add[n_dindex] = c[v]
            vertex_to_delete[ca_dindex] = c[v]
    ...
    for i in changes.keys():
        if i == 0:
            continue
        similar_pop = math.isclose(changes[i], changes[i-1])
    if similar_pop == True:
        for i in changes.keys():
            ...
            districts_graphs[i].add_vertex(vertex_to_add[i])
            districts_graphs[i].remove_vertex(vertex_to_delete[i])
    if similar_pop == False:
        selected_vertices = get_non_adjacent_v(labels_in_boundaries, graph)
        propose_swap(districts_graphs, slected_vertices, graph)
...
gt.graph_draw(graph, pos=graph.vp.pos,
              output=str(main_folder / ('tmp_b.png')),
...
selected_vertices = get_non_adjacent_v(labels_in_boundaries, graph)
propose_swap(districts_graphs, selected_vertices, graph)

gather_districts_data(districts_graphs)
color, ring_color = adjust_color(districts_graphs, color, ring_color)

gt.graph_draw(graph, pos=graph.vp.pos,
              output=str(main_folder / ('tmp_a.png')),
              bg_color=(255, 255, 255, 1), vertex_text=graph.vertex_index, vertex_color=ring_color, vertex_fill_color=color)
