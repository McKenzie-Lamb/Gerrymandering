...
def main(no_districts):
    ...
    state = gt.minimize_blockmodel_dl(graph, 4,4)
    for i in range(10):
        ds, nmoves = state.mcmc_sweep(niter=100)
        print(ds)
        print(nmoves)
        graph.vp.pos = pos
        state.draw(pos=graph.vp.pos, output="abel-network-files/tmp_alg_states"+str(i)+".png")
