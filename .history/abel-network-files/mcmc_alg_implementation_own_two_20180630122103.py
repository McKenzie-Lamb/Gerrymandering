...
def gather_districts_data(districts_graphs):
    for i in range(len(districts_graphs)):
        ...
        districts_graphs[i]["pop"] = population
        districts_graphs[i]["pop"] = 0
        dem_vote = districts_graphs[i].new_graph_property('int')
        districts_graphs[i]["dem_vote"] = dem_vote
        districts_graphs[i]["dem_vote"] = 0
        rep_vote = districts_graphs[i].new_graph_property('int')
        districts_graphs[i]["rep_vote"] = rep_vote
        districts_graphs[i]["rep_vote"] = 0
        for v in districts_graphs[i].vertices():
            districts_graphs[i]["pop"] += graph.vp.data[v]["PERSONS"]
            districts_graphs[i]["dem_vote"] += graph.vp.data[v]["CONDEM14"]
            districts_graphs[i]["rep_vote"] += graph.vp.data[v]["CONREP14"]
