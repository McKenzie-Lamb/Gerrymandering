            ...
            for v in cp:
                ...
                for n in graph.vertex(v).all_neighbors():
                    for g_two in range(len(turned_on_graphs)):
                        if g == g_two:
                            continue
                        if turned_on_graphs[g_two].vertex(n).is_valid() == False:
                            vertex_bound = True
                if vertex_bound == False:
                    break
            if label_boun == False:
                break
            else:
...
labels_in_boundaries = get_cp_boundaries(turned_on_graphs)
pritn(labels_in_boundaries)