...
def get_non_adjacent_v(labels_in_boundaries, graph):
    list_to_swap = random.sample(
        labels_in_boundaries, random.randint(2, len(labels_in_boundaries)//2)) # Prob for choosing from boundaries
    index_to_del = list()
...
def adjust_color(districts_graphs, color, ring_color, niter_type = 'first', ring_colors_dict = None):
    if niter_type == 'nonfirst':
        ...
        for i in range(len(districts_graphs)):
            ring_colors_dict[i] = random_color()
            if districts_graphs[i].graph_properties["dem_vote"] > districts_graphs[i].graph_properties["rep_vote"]:
                color_ = (0, 0, 255, 1)
            else:
                color_ = (255, 0, 0, 1)
...
for i in range(10):
    ...
    for i in districts_graphs:
        comp, hist = gt.label_components(i)
        if sum(comp.a) == 0:
            print(True)
        else:
            print(False)
    ...
    color, ring_color = adjust_color(graph, color, ring_color, niter_type = 'nonfirst', ring_colors_dict = ring_colors_dict)
    gt.graph_draw(graph, vertex_fill_color = color, vertex_color = ring_color,
