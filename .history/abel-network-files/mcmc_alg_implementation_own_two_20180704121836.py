...
def get_non_adjacent_v(labels_in_boundaries, graph):
    list_to_swap = random.sample(
        labels_in_boundaries, random.randint(1, len(labels_in_boundaries))) # Prob for choosing from boundaries
    index_to_del = set()
...
def propose_swap(districts_graphs, proposed_components, graph, labels_in_boundaries):
    ...
    if similar_pop == True:
        ...
        previous_state = dict()
        for i in changes.keys():
            previous_state[i] = gt.GraphView(districts_graphs[i].copy())
            previous_state[i].graph_properties["pop"] = changes[i][0]
            previous_state[i].graph_properties["rep_vote"] = changes[i][1]
            previous_state[i].graph_properties["dem_vote"] = changes[i][2]
            for j in vertex_to_add[i]:
                if len(vertex_to_add[i]) == 0:
                    break
                previous_state[i].vp.valid[j] = True
            for j in vertex_to_delete[i]:
                if len(vertex_to_delete[i]) == 0:
                    break
                previous_state[i].vp.valid[j] = False
            comp, hist = gt.label_components(previous_state[i])
            if np.sum(comp.a) != 0:
                contiguos = False
                break
        if contiguos == True:
            for i in range(len(districts_graphs)):
                districts_graphs[i] = gt.GraphView(previous_state[i].copy())
            return districts_graphs
        else:
