...
def turn_off_edges(districts_graphs):
    ...
    for district in range(len(districts_graphs)):
        ...
        selected = edges[np.random.randint(edges.shape[0], size = 30), :]
        selected_edges[district] = selected
        print(selected_edges[district])
        turned_off_graphs.append(gt.GraphView(districts_graphs[district], efilt=selected_edges[district]))
        print(districts_graphs[district])
        print(turned_off_graphs[district])
...
for i in range(len(districts_graphs)):
    gt.graph_draw(
                  ...
                  output=str(main_folder / ('tmp1'+str(i)+'.png')))
