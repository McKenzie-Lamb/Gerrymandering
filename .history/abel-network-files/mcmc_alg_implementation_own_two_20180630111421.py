...
def get_non_adjacent_v(labels_in_boundaries):
    list_to_swap = random.sample(labels_in_boundaries, random.randint(2,len(labels_in_boundaries)//3))
        ...
        for v in range(len(list_to_swap[l])):
            for l_two in range(len(list_to_swap)):
                ...
                for v_two in range(len(list_to_swap[l_two])):
                    if len(gt.shortest_path(graph, list_to_swap[l][v], list_to_swap[l_two][v_two])) == 0:
                        del(list_to_swap, l)
    return list_to_swap
...
print(labels_in_boundaries)

slected_vertices = get_non_adjacent_v(labels_in_boundaries)
print(slected_vertices)
print(len(gt.shortest_path(graph, graph.vertex(10), graph.vertex(16))))